...
<input type="hidden" name="id" value="{{user._id}}" />
    <table class="table table-bordered table-hover definewidth m10">
        ...
        <tr>
            ...
            <td><input type="text" name="realname" value="{{user.name}}"/></td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td>
                ...
                <input type="radio" name="status" value="0"
                    <eq name="user.status" value='1' {% if user.vstatus == 1 %}{{"checked"}}{% endif %}></eq> 启用
              <input type="radio" name="status" value="1"
                    <eq name="user.status" value='0' {% if user.vstatus == 0 %}{{"checked"}}{% endif %}></eq> 禁用
            </td>
        </tr>
        <tr>
            <td class="tableleft">角色</td>
            <td><input type="text" name="roles" value="{% for app in user.apps %}{% for role in app['roles'] %}{{ role }}{{';'}}{%endfor%}{% endfor %}
              "></td>
        </tr>
        <tr>
