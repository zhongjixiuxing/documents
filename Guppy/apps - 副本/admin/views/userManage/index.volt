...
<table class="table table-bordered table-hover definewidth m10">
    ...
    {% for user in users %}
        ...
        <td>{{user.name}}</td>
        <td></td>
        <td>{% for app in user.apps %}{% for role in app['roles'] %}{{ role }}{{';'}}{%endfor%}{% endfor %}</td>
        <td>
      <a href="/Faeva/admin/UserManage/edit/{{user._id}}">编辑</a> |
        <a href="/Faeva/admin/UserManage/delete/{{user._id}}">删除</a>
      </td>
    {% endfor %}
